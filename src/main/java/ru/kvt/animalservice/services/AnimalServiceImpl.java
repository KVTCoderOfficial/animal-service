...
public class AnimalServiceImpl implements AnimalService {
    ...
    public Page<Animal> findAllByUsername(String username, int page, int pageSize) {
        return animalRepository.findAllByUser(userRepository.findByUsername(username).orElseThrow(() -> new UserNotFoundException(String.format("User '%s' not found", username))), PageRequest.of(page - 1, pageSize));
    }
